...
@section('title', 'Add Treatment - Skin Care Clinic')
...
<div class="mb-6">
    ...
    <h1 class="text-2xl font-bold text-gray-800">Add Treatment</h1>
    <p class="text-gray-500 text-sm mt-1">Create a new treatment available at the clinic.</p>
</div>
        ...
        <form action="{{ route('treatments.store') }}" method="POST">
            ...
            <div class="mb-6">
                ...
                <textarea id="description" name="description" rows="4"
                    ...
                    class="w-full px-4 py-2.5 text-sm border {{ $errors->has('description') ? 'border-red-400 bg-red-50' : 'border-gray-300' }} rounded-lg focus:outline-none focus:ring-2 focus:ring-rose-400 focus:border-transparent resize-none">{{ old('description') }}</textarea>
                @error('description')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
